...
describe('.airspaceValidator', () => {
    ...
    describe('invalid data', () => {
        ...
        it('should return the correct error message', () => {
            ...
            airspaceValidator(foundErrorsMock, {});
    ...
    describe('valid data', () => {
        ...
        it('should call Notifier.succeed()', () => {
            const foundErrorsMock = []
            airspaceValidator(foundErrorsMock, airportKseaMock);

            expect(notifierSucceedSpy.callCount).to.eq(2);
        });

        it('should return true', () => {
            expect(airspaceValidator([], airportKseaMock)).to.be.true;
        });
    });
    ...
    describe('airspace obj keys', () => {
        describe('invalid obj keys', () => {
            ...
            it('should return the error message', () => {
                const foundErrorsMock = [];
                airspaceValidator(foundErrorsMock, invalidObjKeyMock);

                expect(foundErrorsMock[0]).to.eq('FAILED: `airspace` poly section #0 is missing the following keys: ceiling, airspace_class');
            });

            it('should call Notifier.fail()', () => {
                airspaceValidator([], invalidObjKeyMock);

                expect(notifierFailSpy.calledOnce).to.be.true;
            });

            it('should return false', () => {
                expect(airspaceValidator([], invalidObjKeyMock)).to.be.false;
            });
        });
    });

    describe('airspace poly lat/long', () => {
        describe.skip('invalid lat/long format', () => {
            it('should return an error message', () => {
                const foundErrorsMock = [];
                const invalidAirportMock = Object.assign({}, airportKseaMock);
                ...
                invalidAirportMock.airspace = [invalidAirspacePoly];

                expect(airspaceValidator(foundErrorsMock, invalidAirportMock)).to.be.false;
            });
        });
    });
});
