...
describe('.BaseValidator', () => {
    ...
    describe('when extended', () => {
        describe('when passed valid data', () => {
            ...
            it('should call `.registerError` when errors exist', () => {
                const airspaceValidatorWithFakeRequiredKey = new AirspaceValidatorFixture('airspace', airportKseaMock.airspace);
                const registerErrorSpy = sinon.spy(airspaceValidatorWithFakeRequiredKey, 'registerError');

                airspaceValidatorWithFakeRequiredKey._requiredKeys.push('threeve');
                airspaceValidatorWithFakeRequiredKey.validate();

                expect(registerErrorSpy.callCount).to.eq(2);
            });
