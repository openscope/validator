...
class SidsValidator extends ValidatorAbstract {
    ...
    validatePatternFixes() {
        ...
        for (let procedureIcao in this._data) {
            const rwys = this._extractSegmentFromProcedure(PROCEDURE.SEGMENT_NAME.RWY, procedureIcao);
            const body = this._extractSegmentFromProcedure(PROCEDURE.SEGMENT_NAME.BODY, procedureIcao);
            const exit = this._extractSegmentFromProcedure(PROCEDURE.SEGMENT_NAME.EXIT_POINTS, procedureIcao);
